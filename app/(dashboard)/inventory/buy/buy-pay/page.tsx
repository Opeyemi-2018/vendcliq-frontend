...
function BuyPayContent() {
  ...
  const [invoicePreview, setInvoicePreview] =
    useState<BuyInvoicePreview | null>(null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        setInvoicePreview(JSON.parse(saved) as BuyInvoicePreview);
      } catch {
  ...
  const cleanupPreview = () => {
    if (invoiceId) {
      localStorage.removeItem(`buy-invoice-preview-${invoiceId}`);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-8 mt-4 md:mt-8">
        ...
        <div className="lg:hidden flex gap-2 bg-[#ECECF080] p-1 rounded-lg">
          <button
            ...
            className={`flex-1 py-3 rounded-lg font-dm-sans font-medium text-[14px] transition-all ${paymentMethod === "WALLET" ? "bg-[#0A6DC0] text-white" : "text-[#9E9A9A]"}`}
          >
            Cliq Wallet
          </button>
          <button
            onClick={() => setPaymentMethod("TRANSFER")}
            className={`flex-1 py-3 rounded-lg font-dm-sans font-medium text-[14px] transition-all ${paymentMethod === "TRANSFER" ? "bg-[#0A6DC0] text-white" : "text-[#9E9A9A]"}`}
          >
            Bank Transfer
          </button>
        </div>
        ...
        <div className="md:p-6 lg:border border-[#E4E4E4] rounded-lg h-full lg:w-[35%] bg-white hidden lg:block">
          <div className="space-y-4">
            <div
              onClick={() => setPaymentMethod("WALLET")}
              className={`p-4 rounded-lg border cursor-pointer transition mb-4 ${paymentMethod === "WALLET" ? "border-[#0A6DC0] bg-[#0A6DC012]" : "border-gray-200"}`}
            >
              <h3 className="font-medium font-dm-sans">Cliq Wallet</h3>
            </div>
            <div
              onClick={() => setPaymentMethod("TRANSFER")}
              className={`p-4 rounded-lg border cursor-pointer transition ${paymentMethod === "TRANSFER" ? "border-[#0A6DC0] bg-[#0A6DC012]" : "border-gray-200"}`}
            >
        ...
        <div className="md:p-6 lg:border border-[#E4E4E4] rounded-lg lg:w-[65%] bg-white">
          ...
          <p className="font-dm-sans text-[#9E9A9A] mb-4">
            Here is all about the products you want to buy
          </p>
          ...
          {invoicePreview.items.length > 0 && (
            <div className="space-y-3 mb-6">
              <h2 className="font-dm-sans font-bold text-[13px] md:text-[16px]">
                Products
              </h2>
                ...
                >
                  <div className="flex items-center gap-2">
                    {item.product_image ? (
                      ...
                      <div className="w-14 h-14 rounded bg-gray-100 flex-shrink-0 flex items-center justify-center text-gray-400 text-xs border border-gray-200">
                        No img
                      </div>
                    )}
                    <div className="text-[#2F2F2F]">
                      <p className="font-medium text-[15px] font-dm-sans">
                        {item.sku}
                      </p>
                      <p className="text-[13px] text-gray-500">
                        {item.product_name}
                      </p>
                      <p className="text-[13px] font-dm-sans">
                        {item.quantity} {item.mode.toLowerCase()} × ₦
                        {(item.sub_total / item.quantity).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <p className="font-medium text-[15px] font-dm-sans">
          ...
          <div className="mb-6">
            ...
            <Input
              ...
              placeholder="e.g. Payment for purchase"
              className="mt-2 py-6"
            />
          </div>
            ...
            <div className="flex gap-4 justify-center mb-6">
              {[0, 1, 2, 3].map((index) => (
                <div key={index} className="relative">
                  <div
                    className={`w-16 h-16 border-2 rounded-xl flex items-center justify-center text-[16px] font-medium transition-all relative ${transactionPin?.[index] ? "border-[#0A6DC0] bg-[#0A6DC01A]" : "border-[#D8D8D866] bg-[#F9F9F9]"} ${transactionPin?.length === index ? "!border-[#0A6DC0] !bg-white" : ""}`}
                  >
            ...
            <div className="space-y-3 mt-2">
              <div className="bg-[#F9F9F9] border border-[#E0E0E0] rounded-lg p-2 flex justify-between items-start">
                ...
                <Button
                  ...
                  className="text-[#0A6DC0] hover:bg-blue-50"
                >
